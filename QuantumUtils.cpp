...
void SymmetricOrthMatrix(MatrixXd *SOM, MatrixXd *S) {
    ...
    MatrixXd *eval = new MatrixXd(NUM_ORB, NUM_ORB);
    MatrixXd *evec = new MatrixXd(NUM_ORB, NUM_ORB);
...
void NormalizeColumns(MatrixXd *M, MatrixXd *S){
  // Assert that c * S * c = 1
     Matrix<double, 1, NUM_ORB> eigenvec;
    double norm_val;
    for (int i = 0; i < (*M).cols(); i++){
        // pull out eivenvector
        eigenvec = (*M).col(i);
        // normilization value is c * S * c
        norm_val = eigenvec * (*S) * eigenvec.transpose();
...
void atomicToMolecularN5(MatrixXd *TEI_MO, MatrixXd *TEI_AO, MatrixXd *C0) {
      ...
      MatrixXd* X = new MatrixXd(NUM_ORB, NUM_ORB);
      setzero(X);
